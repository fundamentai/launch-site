            ...
            {!onlyNext && (
                <button
                    className={`${btnClassName} ${
                        !onlyPrev ? 'me-2' : ''
                    } border-slate-200 dark:border-slate-600 rounded-full flex items-center justify-center ${focus === 'left' ? 'border-2' : ''}`}
                    onClick={(e) => {
                        e.preventDefault()
                        onClickPrev()
                    }}
                ...
                >
                    <svg className="w-5 h-5 rtl:rotate-180" viewBox="0 0 24 24" fill="none">
                        <path
