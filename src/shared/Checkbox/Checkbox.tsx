...
const Checkbox: FC<CheckboxProps> = ({
    ...
    return (
        <div className={`flex text-sm sm:text-base ${className}`}>
            ...
            />
            {label && (
                <label htmlFor={name} className="pl-2.5 sm:pl-3.5 flex flex-col flex-1 justify-center select-none">
                    <span className={`text-slate-900 dark:text-slate-100 ${labelClassName} ${!!subLabel ? '-mt-0.5' : ''}`}>{label}</span>
